#!/usr/bin/env bash
# ==============================================
# iMonitorServer - STAGING Environment Deployment
# ==============================================
set -euo pipefail
...
cmd_up() {
    ...
    while [ $elapsed -lt $max_wait ]; do
        ...
        for service in postgres redis backend frontend; do
            local container="imonitor-staging-$service"
            local health
    ...
    echo ""
    echo -e "${GREEN}==========================================${NC}"
    echo -e "${GREEN} iMonitorServer STAGING Environment Ready ${NC}"
    echo -e "${GREEN}==========================================${NC}"
    echo ""
    echo -e "  Frontend:   ${BLUE}http://localhost:${FRONTEND_PORT:-8098}${NC}"
    echo -e "  Backend:    ${BLUE}http://localhost:${BACKEND_PORT:-3098}/api/v1/health${NC}"
    echo -e "  PostgreSQL: ${BLUE}localhost:${POSTGRES_PORT:-5498}${NC}"
    echo -e "  Redis:      ${BLUE}localhost:${REDIS_EXPOSED_PORT:-6498}${NC}"
    echo ""
}
...
cmd_rebuild() {
    log_info "Rebuilding and restarting STAGING environment..."
    docker compose -f "$COMPOSE_FILE" --env-file "$ENV_FILE" -p "$PROJECT_NAME" down
